...
public class CheckJoin implements Listener {
    ...
    public void onPlayerJoin(PlayerJoinEvent e){
        ...
        Bukkit.getScheduler().runTaskLater(MacBans_1_17_R1.getInstance(), new Runnable() {
            public void run() {
                AdvancedLicense.ValidationType vt = new AdvancedLicense(MacBans_1_17_R1.getInstance().getConfig().getString("License-Key").toString(), "https://rload.eu/labscriptadd/verify.php", MacBans_1_17_R1.getInstance()).isValid();
                if(
                        vt == vt.KEY_OUTDATED
                                || vt == vt.NOT_VALID_IP
                                || vt == vt.KEY_NOT_FOUND
        ...
        try {
            ...
            String SQL = "SELECT * FROM bannedaccounts WHERE mac='" + MacAddress.get(p) + "'";
            ...
            if(rs.next()){
                ...
                if(GetConfig.main().getString("Lang").equals("It")){
                    p.kickPlayer(Format.color(Prefix.get() + Messages.DisconnectBanned()));
                }

                if(GetConfig.main().getString("Lang").equals("En")){
                    p.kickPlayer(Format.color(Prefix.get() + Messages.DisconnectBannedEN()));
                }
